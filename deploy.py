...
const API = window.location.protocol.startsWith('http') ? window.location.origin : 'http://127.0.0.1:5000';
let map = null;
...
# Strip inline scripts and hook buttons
files_to_patch = ["index.html", "selection.html", "booking.html", "admin.html"]
...
for file_name in files_to_patch:
    if os.path.exists(file_name):
        ...
        # Remove ALL <script> blocks that do not have extremely specific attributes 
        # (We want to keep <script src="theme.js">, <script src="https://...">)
        # Regex to strip inline scripts completely!
        content = re.sub(r'<script>\s*(?!</script>).*?</script>', '', content, flags=re.DOTALL)
        ...
        # Specific wire-ups
        content = content.replace('onclick="bookTrip()"', 'onclick="startBookingFlow()"')
        content = content.replace('onclick="handleAuthAction()"', 'onclick="handleLoginSubmit()"')
        # Ensure form onsubmit triggers handleAddRoute
        content = content.replace('onsubmit="addRoute(event)"', 'onsubmit="handleAddRoute(event)"')

        with open(file_name, "w", encoding="utf-8") as f:
            f.write(content)

print("Deploy successful. 6 files effectively modified seamlessly.")
